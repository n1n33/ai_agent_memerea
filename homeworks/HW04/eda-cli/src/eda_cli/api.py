...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool | int | float | list] = Field(
        ...,
        description="Полный набор флагов и метрик качества данных"
    )
    latency_ms: float = Field(
        ...,
        ge=0.0,
...
@app.post(
    ...
    tags=["quality"],
    summary="Получить все флаги качества из CSV-файла",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    - has_constant_columns (константные колонки)
    - has_suspicious_id_duplicates (дубликаты в ID)
    
    Возвращает все флаги без фильтрации, что полезно для:
    - детальной диагностики качества данных
    - логирования и мониторинга
    - принятия решений о preprocessing
    """
    ...
    # Запускаем EDA-ядро с ПОЛНЫМ набором флагов
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    
    # ВАЖНО: передаем df для активации новых эвристик из HW03
    flags_all = compute_quality_flags(summary, missing_df, df)
    ...
    print(
        ...
        f"has_constant_columns={flags_all.get('has_constant_columns', False)} "
        f"has_suspicious_id_duplicates={flags_all.get('has_suspicious_id_duplicates', False)} "
        f"latency_ms={latency_ms:.1f} ms"
    )
